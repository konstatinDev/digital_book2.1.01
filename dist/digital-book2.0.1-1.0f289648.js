...
document.addEventListener("DOMContentLoaded", ()=>{
    // Elemente
    const arburg1 = document.querySelector("#arburg1");
    const arburg2 = document.querySelector("#arburg2");
    const material = document.querySelector("#material");
    const charge = document.querySelector("#charge");
    const gebinde = document.querySelector("#gebinde");
    const gebindeNr = document.querySelector("#gebindeNr");
    const auftrag = document.querySelector("#auftrag");
    const test_info = document.querySelector("#test_info");
    const test_plus = document.querySelector("#test_plus");
    const plusBtn = document.querySelector('input[type="button"][value="+"]');
    const speichernBtn = document.querySelector('input[type="button"][value="Speichern"]');
    const suchenBtn = document.querySelector('input[type="button"][value="Suchen"]');
    const infoBtn = document.querySelector('input[type="button"][value="Information"]');
    const tableBody = document.querySelector("#tableBody");
    const probenNotes = document.querySelector("#proben_notes");
    // --- Helfer ---
    const formatNow = ()=>{
        const d = new Date();
    ...
    const renderTable = ()=>{
        ...
        const existingGenerated = tableBody.querySelectorAll(".row[data-generated='true']");
    ...
    speichernBtn.addEventListener("click", ()=>{
        ...
        if (!arburgSelected) errors.push("Bitte w\xe4hle Arburg 1 oder Arburg 2.");
        ...
        if (!gebinde.value.trim()) errors.push("Gebinde muss ausgew\xe4hlt sein.");
        if (!gebindeNr.value.trim()) errors.push("Gebinde Nr muss ausgew\xe4hlt sein.");
        if (!auftrag.value.trim()) errors.push("Auftrag muss ausgew\xe4hlt sein.");
        if (!test_plus.value.trim()) errors.push("Bitte mindestens eine Pr\xfcfung ausw\xe4hlen (+ dr\xfccken).");
        if (errors.length > 0) {
            alert("Fehler:\n- " + errors.join("\n- "));
            return;
        }
    ...
    suchenBtn.addEventListener("click", ()=>{
        alert("Suchen: Noch nicht implementiert \u2014 gib Bescheid wenn du Suche nach Feld/Filter m\xf6chtest.");
    });
    infoBtn.addEventListener("click", ()=>{
        alert("Information: Implementiere hier zus\xe4tzliche Detail-Informationen oder Modals nach Bedarf.");
    });
    // Initial render (auch bestehende localStorage Einträge laden)
    renderTable();
});

},{}]},["elbaT","gNc1f"], "gNc1f", "parcelRequirebb14", {})

//# sourceMappingURL=digital-book2.0.1-1.0f289648.js.map
